...
int main(){
    ...
    vector<int> arr(n);
    cout << "Enter values of the tree (-1 for NULL): ";
    for(int i = 0; i < n; i++){
        cin >> arr[i];
    }
    ...
    int maxi=0;
    int ans = sol.maxPathSum(root, maxi);
    if(ans > 0)    cout << "Maximum Path Sum : " << maxi;
    else cout << "The tree contains all negative node values";
    return 0;
}
...
int main(){
    ...
    int x;
    cout << "Enter the value whose successor you want to find out : ";
    cin >> x;

    Solution sol;
    TreeNode* root = sol.buildTree(arr);

    TreeNode* successor = sol.inOrderSuccessor(root, x);

    if(successor)     cout << "Successor of " << x << " in the BST is : " << successor->val;
